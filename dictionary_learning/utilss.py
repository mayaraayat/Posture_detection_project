...
class GTZANDataset(torch.utils.data.Dataset):
    def __init__(self,
                 root='/home/efernand/data/GTZAN/images',
                 domains=None,
                 transform=None,
                 train=True,
                 test=True,
        ...
        self.filepaths, self.labels = [], []
        for domain in self.domains:
            filepaths, labels = self.get_filepaths_and_labels(domain, train=train, test=test, fold=fold)

            self.filepaths += filepaths
            self.labels += labels

    def get_filepaths_and_labels(self, domain, train=True, test=True, fold=0):
        with open(os.path.join(self.root, 'train_test_splits.json'), 'r') as f:
            splits = json.loads(f.read())
        ...
        if train:
            filepaths += splits[domain]['fold {}'.format(fold)]['Train']
        if test:
            filepaths += splits[domain]['fold {}'.format(fold)]['Test']
    ...
    def __getitem__(self, idx):
        ...
        if is_list:
            ...
            if self.transform:
                im = self.transform(im)
            x = im
            label = torch.tensor(self.name2cat[self.labels[idx]]).long()
            y = torch.nn.functional.one_hot(label, num_classes=self.num_classes).float().squeeze()
...
class DomainNetDataset(torch.utils.data.Dataset):
    def __init__(self, root='/home/efernand/data/DomainNet',
                 domains=None, transform=None, train=True, test=True):
        with open(os.path.join(root, 'domain_net_cat2num.json'), 'r') as f:
            self.name2cat = json.loads(f.read())
    ...
    def get_filepaths_and_labels(self, domain, train=True, test=True):
        ...
        filenames = []
        if train:
            with open(os.path.join(self.root, 'folds/{}_train.txt'.format(domain))) as f:
                lines = f.read().split('\n')
                filenames += [n.split(' ')[0].split('/')[2] for n in lines if len(n) > 0]
                labels += [n.split(' ')[0].split('/')[1] for n in lines if len(n) > 0]
        if test:
            with open(os.path.join(self.root, 'folds/{}_test.txt'.format(domain))) as f:
                lines = f.read().split('\n')
                filenames += [n.split(' ')[0].split('/')[2] for n in lines if len(n) > 0]
                labels += [n.split(' ')[0].split('/')[1] for n in lines if len(n) > 0]

        filepaths = [os.path.join(self.root, domain, c, f) for c, f in zip(labels, filenames)]
...
class OfficeHomeDataset(torch.utils.data.Dataset):
    def __init__(self, root='/home/efernand/data/office-home', domains=None,
                 transform=None, train=True, test=True):
        self.name2cat = {'Alarm_Clock': 0,
            'Backpack': 1,
            'Batteries': 2,
            'Bed': 3,
            'Bike': 4,
...
class OfficeHomeMSDADataset(torch.utils.data.Dataset):
    def __init__(self, root='/home/efernand/data/office-home', domains=None,
                 transform=None, train=True, test=True):
        self.name2cat = {'Alarm_Clock': 0,
            'Backpack': 1,
    ...
    def __len__(self):
        return np.min([len(self.filepaths[domain]) for domain in self.filepaths])

    def __getitem__(self, i):
        x, y = [], []
        for domain in self.filepaths:
            xs_i = Image.open(self.filepaths[domain][i])
            if self.transform:
                xs_i = self.transform(xs_i)
            label = self.labels[domain][i]
            label = torch.tensor([self.name2cat[label]]).long()
            ys_i = torch.nn.functional.one_hot(label, num_classes=self.num_classes).float().squeeze()
            x.append(xs_i)
            y.append(ys_i)
...
class CelebADataset(torch.utils.data.Dataset):
    def __init__(self, root, transform=None):
        self.root = root
        self.transform = transform
        self.im_path = os.path.join(self.root, 'img_align_celeba', 'img_align_celeba')
        self.filenames = os.listdir(self.im_path)
        self.attrs = (0.5 * (pd.read_csv(os.path.join(self.root, 'list_attr_celeba.csv')).values[:, 1:] + 1)).astype(int)
    ...
    def __getitem__(self, idx):
        ...
        if is_list:
            ...
            ims = [Image.open(os.path.join(self.im_path, self.filenames[i])) for i in idx]
            if self.transform:
                ims = [self.transform(im) for im in ims]
            x = torch.stack(ims)
...
class DictionaryDADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.Yt_hat is not None:
            yt_hat = self.Yt_hat[indt]
            return xs, ys, xt, yt, yt_hat
